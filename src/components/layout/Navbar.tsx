...
import { Menu, X, Ticket, Theater } from 'lucide-react';
...
const Navbar = ({ scrolled }: NavbarProps) => {
  ...
  return (
    <header className={`top-0 left-0 right-0 mx-8 md:mx-16 z-40 transition-all duration-300 relative ${scrolled ? 'py-2' : 'py-4'}`}>
            ...
            <div className="hidden md:flex items-center space-x-8">
              ...
              {user ? (
                <>
                  ...
                  <button onClick={() => signOut()} className="btn btn-outline py-2 px-4">LOGOUT</button>
                </>
              ) : (
                <NavLink to="/login" className="btn btn-primary py-2 px-4">
